...
```bash
cd examples/csharp
dotnet build MyPaymentService.CliHost
```
...
```bash
dotnet run --project MyPaymentService.CliHost -- authorize --amount 100.00 --currency USD --merchant MERCH-001
```

Or use the compiled executable:

```bash
cd MyPaymentService.CliHost/bin/Debug/net8.0
.\MyPaymentService.CliHost.exe authorize --amount 100.00 --currency USD --merchant MERCH-001
```
...
```bash
dotnet run --project MyPaymentService.CliHost -- get <payment-id>
```

Example:
```bash
.\MyPaymentService.CliHost.exe get d0f55e76-7159-4cb5-8ab6-66526ca6cdf1
```
...
dotnet run --project MyPaymentService.CliHost -- authorize --help
dotnet run --project MyPaymentService.CliHost -- get --help
```
...
The CLI depends on:
- `MyPaymentService.Core` - Domain, SharedKernel, Capabilities, and Application
- `MyPaymentService.Infrastructure.InMemory` - In-memory implementations of capabilities
...
```bash
.\MyPaymentService.CliHost.exe authorize --amount 49.99 --currency USD --merchant MERCH-001
# Exit code: 0
```

**Declined (fraud detection threshold > $5,000):**
```bash
.\MyPaymentService.CliHost.exe authorize --amount 5001.00 --currency USD --merchant MERCH-001
# Exit code: 1
...
```bash
.\MyPaymentService.CliHost.exe authorize --amount 15000.00 --currency USD --merchant MERCH-001
# Exit code: 1
