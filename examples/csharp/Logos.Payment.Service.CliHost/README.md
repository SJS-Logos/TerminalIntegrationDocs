...
```bash
cd examples/csharp
dotnet build Logos.Payment.Service.CliHost
```
...
```bash
dotnet run --project Logos.Payment.Service.CliHost -- authorize --amount 100.00 --currency USD --merchant MERCH-001
```

Or use the compiled executable:

```bash
cd Logos.Payment.Service.CliHost/bin/Debug/net8.0
.\Logos.Payment.Service.CliHost.exe authorize --amount 100.00 --currency USD --merchant MERCH-001
```
...
```bash
dotnet run --project Logos.Payment.Service.CliHost -- get <payment-id>
```

Example:
```bash
.\Logos.Payment.Service.CliHost.exe get d0f55e76-7159-4cb5-8ab6-66526ca6cdf1
```
...
dotnet run --project Logos.Payment.Service.CliHost -- authorize --help
dotnet run --project Logos.Payment.Service.CliHost -- get --help
```
...
The CLI depends on:
- `Logos.Payment.Service.Core` - Domain, SharedKernel, Capabilities, and Application
- `Logos.Payment.Service.Infrastructure.InMemory` - In-memory implementations of capabilities
...
```bash
.\Logos.Payment.Service.CliHost.exe authorize --amount 49.99 --currency USD --merchant MERCH-001
# Exit code: 0
```

**Declined (fraud detection threshold > $5,000):**
```bash
.\Logos.Payment.Service.CliHost.exe authorize --amount 5001.00 --currency USD --merchant MERCH-001
# Exit code: 1
...
```bash
.\Logos.Payment.Service.CliHost.exe authorize --amount 15000.00 --currency USD --merchant MERCH-001
# Exit code: 1
